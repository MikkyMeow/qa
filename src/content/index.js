export const themes = [
    ...
    questions: [
      {
        ...
        answer:
          "Прогрессивный рендеринг - это подход при разработке веб-приложений, который позволяет постепенно отображать контент на странице, улучшая восприятие пользователем.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Веб-компоненты - это набор веб-технологий, позволяющих создавать собственные пользовательские элементы с уникальной функциональностью и стилизацией. Основные технологии включают в себя Custom Elements, Shadow DOM, HTML Templates и HTML Imports.",
      },
      {
        question: "Особенности разработки мультиязычных сайтов?",
        answer:
          "Разработка мультиязычных сайтов требует учета локализации контента, управления различными языками и культурными особенностями, создание гибкой системы перевода, а также обеспечение правильной кодировки и мультиязычных URL-адресов.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Принципы REST-архитектуры включают: клиент-серверная архитектура, отсутствие состояния (stateless), кэширование, единообразие интерфейса, слоистая система и код по требованию (опциональность).",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Основные уровни модели зрелости Ричардсона включают: 0 уровень - использование одного URI и одного метода HTTP, 1 уровень - создание отдельных ресурсов для каждой сущности, 2 уровень - использование стандартных методов HTTP, 3 уровень - использование гипермедиа для определения возможных действий.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "preload - загружает ресурс с высоким приоритетом, prefetch - предварительно загружает ресурс для будущего использования, preconnect - инициирует соединение с сервером для будущих запросов, prerender - загружает и рендерит страницу заранее.",
      },
      {
        question: "Для чего нужен паттерн PRPL?",
        answer:
          "Паттерн PRPL используется для создания быстрых и надежных веб-приложений. Он включает этапы Push, Render, Pre-cache и Lazy-load, оптимизируя загрузку и кэширование ресурсов.",
      },
      {
  ...
  },
  {
    ...
    questions: [
      ...
      {
        question: 'Что такое "распределенная система аутентификации"?',
        answer:
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        answer:
          "Strict mode - это режим, введенный в ECMAScript 5, который обеспечивает более строгую обработку ошибок, делает код безопаснее, устраняет некоторые недочеты и запрещает использование некоторых потенциально опасных конструкций.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "В JavaScript существуют три типа таймеров: setTimeout, setInterval и requestAnimationFrame. setTimeout используется для запуска функции через определенный интервал времени, setInterval для периодического выполнения функции, а requestAnimationFrame для запланированной анимации.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Замыкание (Closure) в JavaScript - это функция, которая имеет доступ к своей лексической области видимости, даже после того как эта функция была возвращена из области видимости, где она была определена. Это позволяет сохранять доступ к переменным функции после завершения выполнения.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Функции высшего порядка в JavaScript - это функции, которые могут принимать другие функции в качестве аргументов или возвращать функции в качестве результата. Они позволяют абстрагировать операции и работать с функциями как с данными.",
      },
      {
        question:
      ...
      },
      {
        ...
        answer:
          'Основное различие между .forEach() и .map() в JavaScript заключается в том, что .forEach() выполняет предоставленную функцию один раз для каждого элемента массива и возвращает undefined, в то время как .map() также выполняет функцию для каждого элемента, но возвращает новый массив с преобразованными элементами на основе результатов вызова функции для каждого элемента. Таким образом, .map() позволяет преобразовать элементы массива и вернуть новый массив, в то время как .forEach() просто выполняет операцию над каждым элементом без изменения самого массива. <sup className="rounded-full text-xs cursor-pointer [&>*]:!text-white h-4 w-4 px-1 bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 hover:dark:bg-zinc-600">[1](https://medium.com/@zestful_satin_grasshopper_759/javascript-map-%D0%BF%D1%80%D0%BE%D1%82%D0%B8%D0%B2-foreach-f2a258564d02)</sup> <sup className="rounded-full text-xs cursor-pointer [&>*]:!text-white h-4 w-4 px-1 bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 hover:dark:bg-zinc-600">[4](https://dev.to/ibrahima92/4-main-differences-between-foreach-and-map-you-should-know-3a0d)</sup> <sup className="rounded-full text-xs cursor-pointer [&>*]:!text-white h-4 w-4 px-1 bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 hover:dark:bg-zinc-600">[5](https://syedjaferk.hashnode.dev/the-classical-comparison-map-vs-foreach)</sup> ',
      },
      {
        question: "Разница между .call(), .apply() и bind()?",
        answer:
          '.call(), .apply() и .bind() - это методы, позволяющие установить контекст (значение this) для функции в JavaScript. .call() и .apply() немедленно вызывают функцию, устанавливая контекст и передавая аргументы, причем различие между ними заключается в способе передачи аргументов (call - через список аргументов, apply - через массив аргументов), в то время как .bind() создает новую функцию с установленным контекстом, но не вызывает ее сразу. <sup className="rounded-full text-xs cursor-pointer [&>*]:!text-white h-4 w-4 px-1 bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 hover:dark:bg-zinc-600">[10](https://byteofdev.com/tips/map-vs-foreach/)</sup> ',
      },
      {
        question: "Почему в JS функции называют объектами первого класса?",
        answer:
          'В JavaScript функции называют объектами первого класса из-за того, что функции в языке могут быть использованы точно так же, как и любые другие объекты. Они могут быть созданы и переданы в качестве аргументов другим функциям, возвращены из других функций, сохранены в переменных и структурах данных. Это означает, что функции в JavaScript обладают всеми свойствами и возможностями, свойственными другим объектам. <sup className="rounded-full text-xs cursor-pointer [&>*]:!text-white h-4 w-4 px-1 bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 hover:dark:bg-zinc-600">[9](https://progler.ru/blog/raznica-mezhdu-ciklami-foreach-i-map-v-javascript)</sup> ',
      },
      {
        question: "Как определить наличие свойства в объекте?",
        answer:
          'Для определения наличия свойства в объекте в JavaScript можно воспользоваться оператором in, использовать метод hasOwnProperty(), и проверять у объекта соответствующее свойство, либо использовать метод Object.prototype.hasOwnProperty.call(obj, prop), чтобы избежать конфликтов с другими объектами, не имеющими метода hasOwnProperty. <sup className="rounded-full text-xs cursor-pointer [&>*]:!text-white h-4 w-4 px-1 bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 hover:dark:bg-zinc-600">[9](https://progler.ru/blog/raznica-mezhdu-ciklami-foreach-i-map-v-javascript)</sup> ',
      },
      {
        question: "Что такое IIFE?",
        answer:
          'IIFE (Immediately Invoked Function Expression) - это шаблон в JavaScript, который представляет собой функцию, которая определяется и немедленно вызывается прямо после определения. IIFE часто используется для создания локальной области видимости и избегания конфликтов имен в глобальной области. <sup className="rounded-full text-xs cursor-pointer [&>*]:!text-white h-4 w-4 px-1 bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 hover:dark:bg-zinc-600">[9](https://progler.ru/blog/raznica-mezhdu-ciklami-foreach-i-map-v-javascript)</sup> ',
      },
      {
        question: "Что такое псевдомассив arguments?",
        answer:
          'Псевдомассив arguments в JavaScript - это объект, похожий на массив, который содержит аргументы переданные функции при вызове. Он не является полноценным массивом, поэтому не имеет методов массивов, но выглядит как массив и содержит доступ к аргументам по индексу и свойству length. <sup className="rounded-full text-xs cursor-pointer [&>*]:!text-white h-4 w-4 px-1 bg-zinc-400 hover:bg-zinc-500 dark:bg-zinc-700 hover:dark:bg-zinc-600">[9](https://progler.ru/blog/raznica-mezhdu-ciklami-foreach-i-map-v-javascript)</sup> ',
      },
      {
        question: "Разница между host-объектами и нативными объектами?",
        answer:
          "Host-объекты предоставляются средой выполнения, например, браузером или Node.js, такие как window, XMLHttpRequest и другие. Нативные объекты - это стандартные глобальные объекты JavaScript, такие как String, Math, RegExp и другие. Host-объекты зависят от среды выполнения и могут различаться, в то время как нативные объекты непосредственно предоставляются самим JavaScript. [6] [7]",
      },
      {
 ...
 [1]`,
      ...
      },
      {
        ...
        answer:
          "Метод Object.create() в JavaScript создает новый объект с указанным прототипом. Первым аргументом передается объект, который будет прототипом для нового создаваемого объекта. Этот метод позволяет явно указать прототип объекта при его создании. [6]",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Типы объектов JavaScript включают в себя объекты, функции, массивы, строки, числа, булевы значения, Symbol и null/undefined.",
      },
      {
        question: "Парадигмы программирования в JavaScript?",
        answer:
          "Парадигмы программирования в JavaScript включают в себя объектно-ориентированное программирование, функциональное программирование и императивное программирование.",
      },
      {
        question: "Типы ошибок в JavaScript?",
        answer:
          "Типы ошибок в JavaScript включают в себя синтаксические ошибки, ошибки времени выполнения (runtime errors) и логические ошибки.",
      },
      {
        question: "Разница между typeof и instanceof?",
        answer:
          "Оператор typeof используется для определения типа данных переменной, в то время как оператор instanceof проверяет, является ли объект экземпляром определенного класса.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "В регулярных выражениях часто используются методы test(), exec(), match(), search(), replace() и split() для работы с текстом и шаблонами.",
      },
      {
        question: "Что такое полифил (polyfill)?",
        answer:
          "Полифил (polyfill) - это техника веб-разработки, которая предоставляет реализацию функциональности, которая отсутствует в старых версиях браузеров, обеспечивая совместимость со стандартами новых API.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Типы функций по способности принимать другие функции включают в себя обычные функции, функции обратного вызова (callback functions), функции высшего порядка (higher-order functions) и стрелочные функции (arrow functions).",
      },
      {
        question:
      ...
      },
      {
        ...
        answer:
          "Метод Array.from() используется для создания нового массива из массивоподобных или итерируемых объектов, таких как строк, коллекции DOM и т.д.",
      },
      {
        question: "Назовите способы преобразования массива в объект?",
        answer:
          "Массив можно преобразовать в объект, используя циклы (например, for...of или forEach), метод reduce() или Object.assign(). Например, можно использовать Object.assign({}, массив), чтобы создать объект из массива, используя значения массива в качестве значений свойств.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Каррирование - это техника функционального программирования, при которой функция с несколькими аргументами преобразуется в последовательность функций, каждая из которых принимает только один аргумент. Это делает функции более гибкими и позволяет частично применять и переиспользовать их.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Отложенная загрузка изображений может быть реализована путем создания элементовдинамически с помощью JavaScript при прокрутке страницы или при других условиях, чтобы загружать изображения только по мере необходимости и повышать производительность.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Конструктор Proxy в JavaScript используется для создания объекта-заглушки, который может перехватывать определённые операции с другим объектом и выполнять пользовательскую логику.",
      },
      {
        question: "Что такое хвостовая рекурсия? Оптимизация рекурсии?",
        answer:
          "Хвостовая рекурсия - это форма рекурсии, при которой рекурсивный вызов функции является последней операцией в функции. Оптимизация рекурсии позволяет избежать переполнения стека путём замены рекурсивного вызова на цикл.",
      },
      {
        question:
          "Что такое и как работает debounce() и throttle() в JavaScript?",
        answer:
          "Функция debounce() в JavaScript используется для задержки вызова функции, пока не пройдёт определённый интервал времени без новых вызовов. Функция throttle() используется для ограничения частоты вызова функции до определённого числа раз в определённый период времени.",
      },
      {
        question:
          "Как в JavaScript работают декораторы? Как они могут быть использованы для модификации поведения классов и методов?",
        answer:
          "Декораторы в JavaScript - это функции, которые позволяют добавлять функциональность к существующим объектам и функциям. Они могут быть использованы для модификации поведения классов и методов, например, для добавления логирования, кэширования и других аспектов.",
      },
    ],
  },
  {
